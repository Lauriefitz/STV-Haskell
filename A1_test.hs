...
numSeatsQ = do
    putStrLn "How many seats are available?"  
    num <- getLine  
    putStrLn ("There are " ++ num ++ "available.")  
...
-- get the frequency of each value in the matrix
...
-- getting the frequency of each distinct value in the column (votes) and * 1000
-- drop the first value as it's the *

abbott = [((show y), snd ((frequency (colN 0 cleanVotes)) !! y) * 1000) | y <- [1..5]]
--abbott = drop 1 (frequency (colN 0 cleanVotes))

balls =  [((show y), snd ((frequency (colN 1 cleanVotes)) !! y) * 1000) | y <- [1..5]]
--balls =  drop 1 (frequency (colN 1 cleanVotes))

burbhm =  [((show y), snd ((frequency (colN 2 cleanVotes)) !! y) * 1000) | y <- [1..5]]
--burbhm = drop 1 (frequency (colN 2 cleanVotes)) 

dMill =  [((show y), snd ((frequency (colN 3 cleanVotes)) !! y) * 1000) | y <- [1..5]]
--dMill =  drop 1 (frequency (colN 3 cleanVotes)) 

eMill =  [((show y), snd ((frequency (colN 4 cleanVotes)) !! y) * 1000) | y <- [1..5]]
--eMill = drop 1 (frequency (colN 4 cleanVotes))

names = [abbott, balls, burbhm, dMill, eMill]
nameStr = [("Abbott"), ("Balls"), ("Burbhm"), ("DMill"), ("EMill")]

-- Creating new final list of vote counts + name of candidate
newVotes = [((nameStr !! y), (names !! y)) | y <- [0..4]]

-- test: returning no 1 votes greater than quota (for test, quota = 90000)
no1 = [(fst (newVotes !! y ), snd (snd (newVotes !! y ) !! 0)) | y <- [0..4], (snd (snd (newVotes !! y ) !! 0)) > 90000]
nameNo1 = fst (no1 !! 0)
countNo1 = snd (no1 !! 0)

surplus = countNo1 - quota